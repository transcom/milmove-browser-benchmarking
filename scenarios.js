  ...
  if (saveReports) {
    ...
    if (measurement === measurementTypes.networkComparison || measurement === measurementTypes.totalDuration) {
      lhReportPath = networkProfileName
        ? `reports/lighthouse-report-${networkProfileName}-${dateString}.json`
        : `reports/lighthouse-report-${dateString}.json`;
      pTracePath = networkProfileName
        ? `reports/performance-trace-${networkProfileName}-${dateString}.json`
        : `reports/performance-trace-${dateString}.json`;
      ...
      lhReportPath = fileSizeName
        ? `reports/lighthouse-report-${fileSizeName}-${dateString}.json`
        : `reports/lighthouse-report-${dateString}.json`;
      pTracePath = fileSizeName
        ? `reports/performance-trace-${fileSizeName}-${dateString}.json`
        : `reports/performance-trace-${dateString}.json`;
